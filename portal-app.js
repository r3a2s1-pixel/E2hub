...
// Safe fallback: if no view is visible after load, show login instead of a blank page
window.addEventListener("load", () => {
  setTimeout(() => {
    ...
    const anyActive =
      loadingView?.classList.contains("active") ||
      loginView?.classList.contains("active") ||
      appView?.classList.contains("active");

    if (!anyActive && loginView) {
    ...
    if (appView?.classList.contains("active")) {
      ...
      const nav = appView.querySelector(".top-nav");
